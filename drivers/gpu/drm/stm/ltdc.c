...
static irqreturn_t ltdc_irq(int irq, void *arg)
{
	...
	regmap_read(ldev->regmap, LTDC_ISR, &ldev->irq_status);
	regmap_write(ldev->regmap, LTDC_ICR, ldev->irq_status);
				    ...
				    struct drm_crtc_state *old_state)
{
	...
	regmap_write(ldev->regmap, LTDC_BCCR,
...
static void ltdc_crtc_mode_set_nofb(struct drm_crtc *crtc)
{
	...
	/* Configure the output format (hw version dependant) */
				     ...
				     struct drm_plane_state *oldstate)
{
	...
	regmap_update_bits(ldev->regmap, LTDC_L1WHPCR + lofs,
			   LXWHPCR_WHSTPOS | LXWHPCR_WHSPPOS, val);

	/* Configures the vertical start and stop position */
	val = ((dr.y2 + 1 + avbp) << 16) + (dr.y1 + 1 + avbp);
	regmap_update_bits(ldev->regmap, LTDC_L1WVPCR + lofs,
	...
	regmap_update_bits(ldev->regmap, LTDC_L1PFCR + lofs, LXPFCR_PF, val);
	...
	regmap_update_bits(ldev->regmap, LTDC_L1CFBLR + lofs,
			   LXCFBLR_CFBLL | LXCFBLR_CFBP, val);

	/* Specifies the constant alpha value */
	val = state->alpha >> 8;
	regmap_update_bits(ldev->regmap, LTDC_L1CACR + lofs, LXCACR_CONSTA, val);
	...
	if (ldev->caps.non_alpha_only_l1) {
		if (is_xrgb(fb->format->format)) {
			val = BF1_CA | BF2_1CA;
			regmap_write(ldev->regmap, LTDC_L1DCCR + lofs, bgcolor);
		} else {
			val = BF1_PAXCA | BF2_1PAXCA;
		}
	}

	regmap_update_bits(ldev->regmap, LTDC_L1BFCR + lofs,
			   LXBFCR_BF2 | LXBFCR_BF1, val);

	/* Configures the frame buffer line number */
	line_number = dr.y2 - dr.y1 + 1;
	regmap_update_bits(ldev->regmap, LTDC_L1CFBLNR + lofs, LXCFBLNR_CFBLN, line_number);
	...
	if (ldev->caps.ycbcr_input && fb->format->is_yuv) {
		switch (fb->format->format) {
		case DRM_FORMAT_NV12:
		case DRM_FORMAT_NV21:
		/* Configure the auxiliary frame buffer address 0 & 1 */
		paddr = (u32)drm_fb_cma_get_gem_addr(fb, state, 1);
		...
		paddr = (u32)drm_fb_cma_get_gem_addr(fb, state, 2);
		regmap_write(ldev->regmap, LTDC_L1AFBA1R + lofs, paddr);

		line_length = ((fb->format->cpp[0] * (dr.x2 - dr.x1 + 1)) >> 1) +
			      (ldev->caps.bus_width >> 3) - 1;
	...
	val |= LXCR_LEN;
	regmap_write(ldev->regmap, LTDC_L1CR + lofs, val);
				      ...
				      struct drm_plane_state *oldstate)
{
	...
	/* disable layer */
	regmap_clear_bits(ldev->regmap, LTDC_L1CR + lofs, LXCR_LEN);
...
int ltdc_load(struct drm_device *ddev)
{
	...
	ldev->regmap = devm_regmap_init_mmio(&pdev->dev, regs, &stm32_ltdc_regmap_cfg);
	if (IS_ERR(ldev->regmap)) {
		DRM_ERROR("Unable to map ltdc registers\n");
