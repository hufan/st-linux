...
struct dcmipp_device {
	...
	struct dcmipp_graph_entity	entity;
...
static int dcmipp_graph_parse(struct dcmipp_device *dcmipp, struct device_node *node)
{
...
static int dcmipp_graph_init(struct dcmipp_device *dcmipp)
{
	...
	ret = dcmipp_graph_parse(dcmipp, dcmipp->dev->of_node);
	if (ret < 0) {
		dev_err(dcmipp->dev, "Failed to parse graph\n");
		return ret;
	}
